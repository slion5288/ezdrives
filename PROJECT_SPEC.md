...
- 路由：`/` `/courses` `/g1` `/login` `/student` `/student/book` `/student/profile` `/student/notifications` `/instructor`；未知路径重定向 `/`。
...
1. Hero 轮播：使用 `public/hero/hero-1.jpg … hero-6.jpg`（用户真实车辆照片，1920 宽；加载失败回退内置 base64），5 秒自动切换 + 圆点手动切换。
2. 如何预约（how-it-works，3 步）
3. 课程预约（courses）：横向滚动全部上架课程；「查看更多课程」→ `/courses` 全课程网格页
...
6. 认识你的教练（instructor）：姓名/评分/经验年限/bio/车辆
7. 学员评价（testimonials）· 常见问题（FAQ）· CTA 横幅 · 页脚（联系方式：邮箱/电话/地点）
