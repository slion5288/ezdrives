## Change 28 — 恢复教练端旧样式 + 管理员中文内容管理 + 邮件模板改版（已部署）

- **用户反馈**：教练后台底部菜单要回到以前全部展开的样子（7 项）；点击学生课程要回到以前"小方框、点击出详情"的设计；教练密码恢复 123456 方便测试；admin 用 slion/528830；管理员主页内容管理补全 + 只要中文；邮件通知只要中文 + 英文自动翻译 + 全部模板可见 + 所有邮件带 logo 和联系信息。
- **恢复**：教练端 7 项底栏（总览/日程/课程/学员/支付确认/通知/设置，移除 5 项+更多 和「今天」页）；日历恢复旧的小方框周网格（移除移动端时间胶囊，学生/教练一致，点击卡片出详情）。
- **账号**：教练密码恢复 123456（本地+线上，后续自行改为正式密码）；线上 admin 恢复 slion/528830。
- **管理员主页内容**：文案管理补全到 44 项（含顶部徽章、主按钮、步骤标题/副标题、课程/视频栏目标题、G1 栏目、教练栏目标题、学员评价标题、FAQ 标题等）；只显示中文输入，英文预览移除，统一提示"保存时自动翻译为英文"。
- **邮件模板（28 个全部列出）**：管理员只填中文（subject_zh/body_zh），保存时浏览器端自动翻译英文（管理员本机 IP，可靠；服务端兜底），重建双语邮件；0013 迁移预置全部模板的中文初始内容；所有邮件自动附带公司 logo（/logo.png）+ 联系信息页脚（notification.js 统一注入）。
- **修改文件**：InstructorDashboardPage.tsx、helpers.ts、WeekCalendar.tsx、calendar.css、AdminPage.tsx、AdminTemplates.tsx、functions/api/admin/templates/index.js、functions/lib/notification.js、migrations/0013_templates_zh.sql、public/logo.png、i18n ×2、scripts/gen-templates-zh.py。
- **测试**：API 16/16、全流程 ALL PASS、UI 33/33（含 7 项底栏、旧日历网格、中文管理员保存链路）、线上验证通过（admin 登录、模板中文保存、logo.png、bundle 哈希一致）。

## Change 27 — 现代简约手机优先改版 + P0/P1/P2 修复（已部署 2026-08-29）

- **用户要求**：不改变任何业务流程的前提下，把全站改为现代简约、手机优先（暖纸底 #F4F1EC / 墨色字 / 系统字体 / 正文 16px / 点击区 ≥44px），并修复 P0/P1/P2 线上问题。**已部署到 ezdrives.net**（用户明确批准部署）。
- **二次反馈（已应用）**：强调色改用 **logo 配色**（蓝→紫渐变 #8B5CF6→#D300E6、品红紫主色 #A21CAF，不用绿色）；学生后台预约时间页**删除顶部「我的订单」**（只显示已购课程、不显示金额，改为在课程位显示状态）；**选择课程方框统一等宽等高等对齐**（主流设计）；**课间休息提示移到日历视图下方**，课程选择与日历贴合。
- **设计改版**：
