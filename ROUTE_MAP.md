...
- 用途：管理员维护主页展示内容（文字 / 图片 / 教练）。
- 谁可访问：仅已登录管理员（未登录显示登录表单；用户名 `slion`）。
- 进入方式：直接访问 `https://ezdrives.net/admin`。
- 内部页签（非路由）：
  - 主页文字：29 个可编辑字段（中英双语；留空 = 用默认文案）
  - 主页图片：6 张 hero 轮播图上传（压缩为 data URL 存储）
  - 教练：新增/编辑/删除教练（姓名/简介中英/经验年数/照片）；列表为空时首页显示单人档案，非空时首页显示多人卡片
- 返回：页面内「← Back」→ `/`。
- 数据接口：`POST /api/admin/login`、`GET/PUT /api/admin/content`（Bearer token）、`GET /api/public/home`（公开读）。
